...
: AbstractPresenter<StoryDetailView>(view, interactorComponent), StoryDetailPresenter {
  ...
  override fun onAttachToView() {
    ...
    with(viewSubscription) {
      ...
      add(view.buttonBarLeftActionClicks
                  ?.map { linkDrawerOpen }
                  ?.subscribe { if (it) webBack() else navigateUp() })

      add(view.buttonBarRightActionClicks
                  ?.map { linkDrawerOpen }
                  ?.subscribe { if (it) webForward() else navigateDown() })
  ...
  private fun webBack() = view.webViewBack()

  private fun webForward() = view.webViewForward()
